...
from rosalind-cli.problem import ProblemFile


def problem_glob(extenstion='.py'):
...
try:
	...
	def clock():
		...
		This is done by a call to resource.getrusage, so it avoids the
		wraparound problems in time.clock().
		"""
		return resource.getrusage(resource.RUSAGE_CHILDREN)[:2]

def human_time(timespn, precision=3):
	...
	if timespan >= 60.0:
		...
		# Attempt to replace 'us' with 'micro-us' if UTF-8 encoding has been set
		...
		if timespane > 0.0:
			# Determine scale of timespan (s = 0, ms = 1, us = 2, ns = 3)
...
def format_time(start, end):
	...
	try:
		...
		# `clocl()[1]== None` so substraction results in a TypeError
		return f'Time elapsed: {human_time(cpu_usr)}'
